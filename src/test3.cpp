...
int main() {
  ...
  for (int i = 0; i < NUMBER_BLOCKS_ONE; i++) {
    obj1ptr[i] = new Obj1();
    if (obj1ptr[i] == NULL) {
      cout<<"Could not allocate req blocks pool 1"<<endl;
    }
  }
  obj1ptr[NUMBER_BLOCKS_ONE] = new Obj1();
  if (obj1ptr[NUMBER_BLOCKS_ONE] != NULL) {
    cout<<"Could allocate more than req number of blocks pool 1"<<endl;
  }
  ...
  for (int i = 0; i < NUMBER_BLOCKS_TWO; i++) {
    obj2ptr[i] = new Obj2();
    if (obj2ptr[i] == NULL) {
      cout<<"Could not allocate req blocks pool 2"<<endl;
    }
  }
  obj2ptr[NUMBER_BLOCKS_TWO] = new Obj2();
  if (obj2ptr[NUMBER_BLOCKS_TWO] != NULL) {
    cout<<"Could allocate more than req number of blocks pool 2"<<endl;
  }
  ...
  for (int i = 0; i < NUMBER_BLOCKS_THREE; i++) {
    obj3ptr[i] = new Obj3();
    if (obj3ptr[i] == NULL) {
      cout<<"Could not allocate req blocks pool 3"<<endl;
    }
  }
  obj3ptr[NUMBER_BLOCKS_THREE] = new Obj3();
  if (obj3ptr[NUMBER_BLOCKS_THREE] != NULL) {
    cout<<"Could allocate more than req number of blocks pool 3"<<endl;
  }

  cout<<"Start obj4"<<endl;
  Obj4 *obj4 = new Obj4();
  if (obj4 != NULL) {
    cout<<"Able to allocate bigger chunks than allowed pool 4"<<endl;
  }
  ...
  if (obj3ptr[NUMBER_BLOCKS_THREE - 1] == NULL) {
    cout<<"Not deleted properly"<<endl;
  }
  obj3ptr[NUMBER_BLOCKS_THREE] = new Obj3();
  if (obj3ptr[NUMBER_BLOCKS_THREE] != NULL) {
    cout<<"Could allocate more after delete"<<endl;
  }
  ...
  delete obj3ptr[NUMBER_BLOCKS_THREE - 1];
  if (obj3ptr[NUMBER_BLOCKS_THREE] != NULL) {
    cout<<"Could allocate more after invalid delete"<<endl;
  }
